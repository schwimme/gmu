...
int main(int argc, char* argv[])
{
	...
	cv::Mat img_source = cv::imread("Lenna.png", CV_LOAD_IMAGE_COLOR);

	if (!img_source.data)
	{
		std::cerr << "Input image could not be loaded.";
		exit(1);
	}

	cv::Mat img_source_fp;
	img_source.convertTo(img_source_fp, CV_32FC3);

	cv::Mat img_source_lab;
	cv::cvtColor(img_source_fp, img_source_lab, cv::COLOR_BGR2Lab);
